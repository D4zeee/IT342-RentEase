  ...
  useEffect(() => {
    ...
    const fetchNotifications = async () => {
      try {
        ...
        const response = await axios.get(`${API_BASE_URL}/payment_reminders`, {
      ...
      {isLoading ? (
        ...
        <div className="space-y-4">
          {notifications.map((notification) => (
            <Card
              key={notification.id}
              className={notification.status === "unread" ? "border-l-4 border-blue-500 bg-blue-50" : ""}
            >
              <CardBody>
                <Typography color="blue-gray">{notification.message}</Typography>
                    ...
                    onClick={async () => {
                      ...
                      try {
                        const response = await axios.post(
                          `${API_BASE_URL}/rented_units/initiate-payment`,
                          {
