...
export default function SuccessPage() {
  ...
  useEffect(() => {
    if (paymentIntentId) {
      axios
        .get(`${API_BASE_URL}/payments/intent/${paymentIntentId}`)
        .then((response) => {
    ...
    <div className="flex flex-col justify-center items-center min-h-screen p-6">
      ...
      {paymentInfo ? (
        <div className="text-center space-y-4">
          <p className="text-xl">
            Amount Paid: <strong>₱{(paymentInfo.amount / 100).toFixed(2)}</strong>
          </p>
          <p className="text-lg">
            Payment Status: <strong>{paymentInfo.status}</strong>
          </p>
